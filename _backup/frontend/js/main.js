...
class AdditionGame {
  ...
  loadPersistentData() {
    // Data already loaded in constructor, but this method allows init.js to proceed
    return true;
  }
  ...
  speak(text, onEndCallback, repeatCountOverride) {
    ...
    if (this.speechSynthesis && !this.settings.quietMode) {
      const speakWithVoice = (onEndCallback, repeatCountOverride) => {
        ...
        console.log("Selected voice:", preferred ? preferred.name : "none");

        setTimeout(() => {
          console.log("About to create utterances");
          console.log(
            "speechSynthesis state: speaking=",
          ...
          for (let i = 0; i < repeatCount; i++) {
            ...
            console.log(
              "Utterance voice:",
              utterance.voice ? utterance.voice.name : "none"
            );
            utterance.rate = 0.75; // Slightly faster for clarity
            utterance.pitch = 1.1; // Lower pitch for clarity
            utterance.volume = 1.0;
            utterance.onstart = () => {
              console.log("Speech started for utterance", i);
            };
            utterance.onend = () => {
              console.log("Speech ended for utterance", i);
              finishedCount++;
              if (
            ...
            console.log("Speaking utterance", i);
